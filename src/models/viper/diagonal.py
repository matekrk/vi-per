...
class DiagonalVIModel(BaseVIModel):
    ...
    def compute_negative_log_likelihood(self, X_batch, y_batch, mc=False, n_samples=1000):
        ...
        Returns:
            torch.Tensor: A list (of length self.K) of NLL components for each output.
        """
    ...
    def to(self, *args, **kwargs):
        ...
        Ensures all internal parameters are properly moved.

        Args:
            *args: Arguments to pass to torch.nn.Module.to()
            **kwargs: Keyword arguments to pass to torch.nn.Module.to()

        Returns:
            DiagonalVIModel: Self with parameters moved to the specified device
        """
