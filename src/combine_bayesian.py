...
def forward_batch(model, criterion, inputs, targets, opt, phase):
    ...
    if model.binary_squeezed:
        ...
        if phase in ["Train"]:
            ...
            loss = output.val_loss_fn(targets, opt.n_samples)
...
def forward_dataset(model, criterion, data_loader, opt):
    ...
    for i, data in enumerate(data_loader):
        ...
        sum_batch += 1
        inputs, targets = data
        prediction, loss, loss_list = forward_batch(model, criterion, inputs, targets, opt, "Validate")
...
def train(model, criterion, train_set, val_set, opt, labels=None):
    ...
    finetune_params = modify_last_layer_lr(model.named_parameters(), 
                                            ...
                                            bayesian=True) #FIXME: should handle lr_mult as well
    # define optimizer
    ...
    for epoch in range(opt.sum_epoch):
        ...
        for i, data in enumerate(train_set):
            ...
            # train 
            inputs, targets = data
            predictions, loss, loss_list = forward_batch(model, criterion, inputs, targets, opt, "Train")
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
