...
from utils import MultiBinaryBCELoss, WebVisualizer, print_loss, print_accuracy, print_metrics, tensor2im, opt2file
...
def forward_batch(model, criterion, inputs, targets, opt, phase):
    ...
    if model.binary_squeezed:
        ...
        for mini_loss in whole_loss:
            loss_list.append(mini_loss.item())
            loss += mini_loss
    else:
        target_vars = list()
...
def main():
    ...
    if opt.binary_squeezed:
        criterion = MultiBinaryBCELoss(weight=opt.loss_weight, reduction="mean")
    else:
